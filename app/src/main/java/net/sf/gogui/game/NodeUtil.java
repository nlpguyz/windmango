...
public final class NodeUtil
{
    ...
    public static ConstNode findNextByMove(ConstNode node, Move move) {
    ...
    public static ConstNode getMatchChildNode(ConstNode node) {
    	if (node == null) {
    		return null;
    	}
    	...
    	for (int i=0; i<father.getNumberChildren(); i++) {
    		ConstNode child = father.getChildConst(i);
    		Log.i("nodeUtil", "node=" + node.getMove().toString() + ", child=" + child.getMove().toString());
    		if (node.getMove().equals(child.getMove())) {
    			return child.getChildConst();
    		}
    	}
